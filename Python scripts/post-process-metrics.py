...
fp= []
fn = []
for a in range(len(inference_images)):
    print(a)
    nii_gt_path, nii_pred_path = os.path.join(GT_labels_folder,inference_images[a]),os.path.join(processed_inference_folder,inference_images[a])
    nii_gt_path = plb.Path(nii_gt_path)
    nii_pred_path = plb.Path(nii_pred_path)
    dice_sc, false_pos_vol, false_neg_vol = compute_metrics(nii_gt_path, nii_pred_path)
